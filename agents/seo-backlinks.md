...
1. Check credentials: `seogeo backlinks-auth --check --json`
...
5. Format output to match seo-geo-skill conventions

## Tier-Based Workflow

### Tier 0 (Always Available, No Config Needed)
- Common Crawl domain metrics: `seogeo commoncrawl <domain> --json`
  - PageRank, PageRank rank, harmonic centrality, harmonic centrality rank, crawl/ranking presence
- If known backlinks provided, verify them: `seogeo verify-backlinks --target <url> --links <file> --json`
...
### Tier 1 (+ Moz API)
- All Tier 0 checks
- Moz URL metrics: `seogeo moz metrics <url> --json`
  - DA, PA, Spam Score, link counts, referring domains
- Moz referring domains: `seogeo moz domains <url> --json`
- Moz anchor text: `seogeo moz anchors <url> --json`
- Moz top pages: `seogeo moz pages <domain> --json`
...
- All Tier 1 checks
- Bing inbound links: `seogeo bing links <url> --json`
- For comparison between two properties registered to the same Bing account:
  `seogeo bing compare <url1> <url2> --json`
...
Match existing seo-geo-skill patterns:
...
Use `seogeo render <URL> --mode auto --json` for page HTML. `auto` does a raw fetch and only spins up headless Chrome when an SPA shell is detected; `--mode always` forces a render and `--mode never` stays on raw HTTP. The JSON reports `is_spa`, `raw_word_count`, and `rendered_word_count` so you can see exactly how much content JavaScript adds. Use `--a11y-tree` for the landmark and heading outline. Every fetch goes through the SSRF-safe layer (`seogeo url-safety`); never fetch a user-supplied URL by other means.
