...
- `seogeo drift baseline <url>` -- capture a new baseline
- `seogeo drift compare <url>` -- compare current state to baseline
- `seogeo drift history <url>` -- show change history
- `seogeo drift report <file> --output report.html` -- generate HTML report

Never use curl, wget, or raw HTTP requests. All fetching is handled by
`seogeo fetch` internally, which validates URLs against private/loopback
IP ranges.
