...
2. Analyze product pages using `seogeo render --mode auto` and `seogeo parse`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `seogeo dataforseo-merchant`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
seogeo dataforseo-costs check <endpoint>
```
...
```bash
seogeo dataforseo-costs log <endpoint> <actual_cost>
```
