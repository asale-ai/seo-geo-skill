...
1. Check credentials: `seogeo google-auth --check --json`
2. Determine tier (0 = API key, 1 = + service account, 2 = + GA4)
3. Execute tier-appropriate analysis
4. Format output to match seo-geo-skill conventions

## Tier-Based Workflow

### Tier 0 (API Key Only)
- Run PSI + CrUX on homepage: `seogeo pagespeed <url> --json`
- Run CrUX History for origin: `seogeo crux-history <origin> --origin --json`
- Report CWV field data with traffic-light ratings

### Tier 1 (+ Service Account)
- All Tier 0 checks
- GSC top queries/pages (28 days): `seogeo gsc-query --property <prop> --json`
...
- URL Inspection on homepage + key pages: `seogeo gsc-inspect <url> --json`
- GSC sitemap status: `seogeo gsc-sitemaps --property <prop> --json`

### Tier 2 (Full)
- All Tier 1 checks
- GA4 organic traffic (28 days): `seogeo ga4-report --property <id> --json`
- Top organic landing pages: `seogeo ga4-report --property <id> --report top-pages --json`
...
- Generated PDF/HTML/XLSX reports under `output_dir/` by passing `--output-dir "$output_dir"` to `seogeo google-report`
