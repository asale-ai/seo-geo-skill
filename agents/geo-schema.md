...
**IMPORTANT:** WebFetch converts HTML to markdown and strips `<head>` content, which removes JSON-LD blocks. For schema detection, use the seogeo fetch script instead:
```bash
...
Fetch the target URL using `seogeo fetch` (see above) and scan the full HTML source for structured data in all three formats:
