...
**Runtime:** Every analysis runs through the `seogeo` binary — one static
executable, no interpreter and no virtualenv. Run `seogeo commands` for the
full subcommand list, and `seogeo <command> --help` for any one of them.
Prefer `--json` whenever you will parse the output.

Comprehensive SEO analysis across all industries (SaaS, local services,
e-commerce, publishers, agencies). Orchestrates 30 SEO sub-skills and 18 sub-agents, alongside the GEO skill
family (`/geo ...`). A separate optional Firecrawl
...
| `/seo updates` | Google ranking-update timeline from primary sources |
| `/seo doctor` | Check which credentials and optional tools are available |

## Runtime Setup

There is nothing to set up beyond the binary itself. Verify it:
...
Some commands need API credentials. Check what is configured before
promising data you cannot fetch:

```bash
seogeo google-auth --check --json
seogeo backlinks-auth --check --json
```

If a credential is missing, say which one and what it unlocks. Never
improvise an install.
...
3. If Google API credentials detected (`seogeo google-auth --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`seogeo backlinks-auth --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`seogeo drift history <url>`), also spawn seo-drift agent
...
After any analysis command completes, offer to generate a PDF report via `seogeo google-report`.

## Synthesis Methodology

Audits are not just findings, they are findings synthesized into a coherent
strategy. seo-geo-skill uses a 10-principle thinking framework grouped into four
...
Every command below takes `--json`. Use it whenever you will parse the
output; the human-readable form is lossy on purpose.

```bash
seogeo commands                 # the full subcommand list
seogeo <command> --help         # flags for any one of them
seogeo seo-updates --since 2025-01-01 --json
```

`seo-updates` answers "did a Google update cause this?" from a bundled,
primary-source-only timeline. Every entry cites a Google-owned URL. When a
user asks about a ranking change, check the dates before speculating —
and if nothing lines up, say that rather than reaching for the nearest
core update.
