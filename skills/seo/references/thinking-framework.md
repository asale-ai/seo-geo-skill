...
This is the canonical methodology seo-geo-skill uses to assemble raw findings
...
- Raw HTML + rendered HTML (via `seogeo render`)
...
- Core Web Vitals field data from CrUX (via `seogeo pagespeed`)
...
- A PDF via `seogeo google-report` when the user asks for one.
...
- Surface what seo-geo-skill itself **could not measure** (offline
...
These principles are seo-geo-skill's; they are not the user's. Surface them
for the user when:
