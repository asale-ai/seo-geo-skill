...
- **Agent-centric accessibility** — reuses three a11y audits: *Names and labels*
  ...
  1–2 above and the `seogeo agent-ux` heuristics.
...
this category** — `seogeo pagespeed` will not contain agentic results
...
`seogeo render --mode auto` already loads pages headlessly; extending
it with an accessibility-tree dump (`page.accessibility.snapshot()` in
headless Chrome) is the natural place to land an automated agent-UX check in a
future iteration.
