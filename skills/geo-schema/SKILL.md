...
1. Fetch the target page HTML using `seogeo fetch` (see note below)
...
**IMPORTANT:** WebFetch converts HTML to markdown and strips `<head>` content, which removes JSON-LD blocks. Use `seogeo fetch` instead:
```bash
seogeo fetch <url> --json
```
...
seogeo schema-generate organization --name "Example Corp" \
  --url https://example.com --same-as https://www.wikidata.org/wiki/Q123
```

For AI discoverability the priority order differs from classic SEO. Entity
resolution comes first: an `Organization` or `Person` with a `sameAs` array
pointing at Wikipedia, Wikidata, LinkedIn, and GitHub lets a model connect
the page to an entity it already knows. Rich-result eligibility is a
secondary benefit.

Check `retired_for_google_rich_results` in the output before recommending
removal — those types are retired for *Google Search rich results*, not
for machine readability generally.
