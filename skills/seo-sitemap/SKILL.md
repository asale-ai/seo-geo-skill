...
- **No sitemap found**: Run `seogeo sitemap-discovery` and report "not found" only
...
- Sitemaps declared in robots.txt may legitimately live on another host.
  Those are fetched, but flagged with `cross_host: true`.
- Query strings are stripped from displayed URLs and the entry is marked
  `query_redacted`, so a signed sitemap URL never lands in a report.

`crawl-sitemap` follows `<sitemapindex>` entries into their child sitemaps
