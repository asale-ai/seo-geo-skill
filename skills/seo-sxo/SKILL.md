...
1. Fetch the target URL via `seogeo render --mode auto` (SPA-aware and SSRF-safe)
2. Parse with `seogeo parse` to extract: title, H1, meta description,
...
- [ ] Target URL was fetched via `seogeo render --mode auto` (not raw curl/fetch)
