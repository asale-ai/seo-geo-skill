...
**Method 1 — entity API check (run this FIRST, most reliable):**
```bash
seogeo brand-scan "[Brand Name]" --domain example.com --json
```

The command queries the Wikipedia and Wikidata APIs directly and returns
`platforms.wikipedia.has_wikipedia_page`, `has_wikidata_entry`, and the
Wikidata `Q`-id when one exists. Those three fields are the ground truth —
a manual search that "looks empty" is not evidence of absence.
