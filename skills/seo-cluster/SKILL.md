...
instead of WebSearch for SERP data. Run `seogeo dataforseo-costs check serp_organic_live_advanced --count N`
...
- All URLs fetched via `seogeo render <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
