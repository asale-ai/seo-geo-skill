...
`validate` exits non-zero when the file is missing or malformed, and
reports `has_title`, `has_description`, `has_sections`, `has_links`,
`section_count`, `link_count`, plus concrete `issues` and `suggestions`.
It also checks whether `/llms-full.txt` exists.

`generate` writes two files: the concise `llms.txt` (top 10 links per
section) and `llms-full.txt` (every discovered link, each annotated with
that page's meta description). Descriptions are fetched only for
same-origin pages — cross-origin links are listed without one rather than
followed.

Review the generated file before deploying it. The generator categorises
pages by URL pattern, which is a good first pass and a poor final answer:
move anything miscategorised, and replace the placeholder contact email.
