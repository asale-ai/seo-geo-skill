...
# Firecrawl Extension for SEO + GEO Skills
...
`seogeo fetch` because it executes JS and waits for dynamic content.
...
**When to use scrape vs seogeo fetch:**
| Scenario | Use |
|----------|-----|
| Static HTML page | `seogeo fetch` (no API cost) |
| JS-rendered SPA | `firecrawl_scrape` (renders JS) |
| Need response headers | `seogeo fetch` (returns headers) |
...
1. Use `seogeo fetch` for single-page analysis (no API cost)
