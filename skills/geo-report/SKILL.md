...
```bash
# Structured audit JSON -> client-ready HTML or PDF
seogeo google-report --type geo --data geo-audit.json \
  --domain example.com --format pdf --output-dir ./example-com/

# Markdown report -> styled PDF deliverable
seogeo report-pdf GEO-AUDIT-REPORT.md -o example-com-geo-report
```

Use `google-report` when the audit produced JSON, and `report-pdf` when it
produced markdown. Both render self-contained files — no external CSS, no
webfonts, nothing that breaks when the client opens it offline.

`google-report` renders arbitrary nested JSON: objects become sections,
arrays of objects become tables, scalars become labelled values. That means
you can hand it whatever shape your audit produced without writing a
template first.
