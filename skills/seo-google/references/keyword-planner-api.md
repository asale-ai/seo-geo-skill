...
Add to `~/.config/seo-geo-skill/google-api.json`:
...
seogeo keyword-planner volume "espresso machine" "burr grinder" --json
```

Both subcommands call the Ads REST API directly. They need three things
configured, and report exactly which one is missing if not:

| Setting | Environment variable |
|---------|----------------------|
| Developer token | `GOOGLE_ADS_DEVELOPER_TOKEN` |
| Customer ID (digits only) | `GOOGLE_ADS_CUSTOMER_ID` |
| Service account with Ads access | `GOOGLE_APPLICATION_CREDENTIALS` |
