...
3. Name it (e.g., "SEO + GEO Skills") and note the project ID
...
2. Name: `seo-geo-skill` (or similar)
3. Skip optional permissions steps
4. Click on the created service account > **Keys > Add Key > Create new key > JSON**
5. Download the JSON file and store it securely (e.g., `~/.config/seo-geo-skill/service_account.json`)
...
```bash
mkdir -p ~/.config/seo-geo-skill
```

Save to `~/.config/seo-geo-skill/google-api.json`:

```json
{
  "service_account_path": "~/.config/seo-geo-skill/service_account.json",
