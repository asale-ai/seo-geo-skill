...
- **GBP chat CTAs** (Critical). Google fully sunset chat and call history on
  2024-07-31. A "Message us on Google" button now does nothing, which is
  worse than having no button.
- **`*.business.site` URLs** (Medium). These are legacy GBP-hosted sites.
  Verify whether the URL still resolves before advising a change.

The lint deliberately does not flag generic "Message us" or "Chat" CTAs —
those are normal. It only matches phrasings that name Google Business.
