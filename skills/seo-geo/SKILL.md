...
Read `references/llmstxt-evidence.md` for the primary-source evidence (Mueller, Illyes, SE Ranking 300k-domain study, OtterlyAI server-log audit) on why `/llms.txt` is not currently a citation lever for major AI search systems. seo-geo-skill reports presence but assigns no citation-ranking weight.
...
seogeo citability https://example.com/page --json

# Can the AI crawlers reach the site at all? (robots.txt + a live fetch per UA)
seogeo robots https://example.com --json

# llms.txt: validate what exists, or generate one from the site
seogeo llms-txt validate https://example.com --json
seogeo llms-txt generate https://example.com --max-pages 30 -o llms.txt

# Is the page readable without JavaScript?
seogeo agent-ux https://example.com/page --json

# Brand presence on the platforms AI answers cite most
seogeo brand-scan "Example Corp" --domain example.com --json
```

Run `robots` before anything else. `ai_crawler_status` reports what
robots.txt says; `live_crawler_fetch` reports what the server actually
does when asked with that user agent. A site can allow GPTBot in
robots.txt and still return 403 to it at the CDN — that gap is invisible
to every robots.txt-only checker and is the single most common cause of
"we have great content but no AI citations".
