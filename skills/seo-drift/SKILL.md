...
2. Fetch page via `seogeo fetch`
3. Parse HTML via `seogeo parse`
4. Optionally fetch CWV via `seogeo pagespeed` (use `--skip-cwv` to skip)
5. Hash HTML body and schema content (SHA-256)
6. Store snapshot in SQLite

**Execution:**
```bash
seogeo drift baseline <url>
seogeo drift baseline <url> --skip-cwv
```
...
**Execution:**
```bash
seogeo drift compare <url>
seogeo drift compare <url> --baseline-id 5
seogeo drift compare <url> --skip-cwv
```
...
```bash
seogeo drift report <comparison_json_file> --output drift-report.html
```
...
**Execution:**
```bash
seogeo drift history <url>
seogeo drift history <url> --limit 10
```
...
- **All URL fetching** goes through `seogeo fetch` which enforces SSRF protection
