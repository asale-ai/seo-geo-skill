...
```bash
# Record each audit as you run it
seogeo crm audit example.com --score 62 --report ./audit-2026-05/GEO-AUDIT-REPORT.md
seogeo crm audit example.com --score 74 --report ./audit-2026-06/GEO-AUDIT-REPORT.md

# Month-over-month delta
seogeo crm compare example.com --json
```

`crm compare` uses the two most recent stored audits and returns `delta`,
`delta_pct`, `direction` (`improved` / `regressed` / `flat`), and a
one-line `summary` suitable for the top of a client email.

It errors when fewer than two audits exist rather than inventing a
baseline. If that happens, say so plainly — a delta report with one data
point is not a delta report.

For page-level regressions between audits, pair this with drift:

```bash
seogeo drift compare https://example.com --json
```
