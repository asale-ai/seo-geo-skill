...
The output has two independent halves and both matter:

| Field | What it answers |
|-------|-----------------|
| `ai_crawler_status` | What robots.txt *permits* for each AI crawler token |
| `live_crawler_fetch` | What the origin *actually returns* to that user agent |
| `sitemaps` | Sitemap URLs declared in robots.txt |

Status values: `ALLOWED`, `BLOCKED`, `PARTIALLY_BLOCKED`,
`ALLOWED_BY_DEFAULT` (no rule for this agent, wildcard permits),
`BLOCKED_BY_WILDCARD`, `NOT_MENTIONED`, `NO_ROBOTS_TXT`.

An agent listed `ALLOWED` with `served: false` in `live_crawler_fetch` is
being blocked below robots.txt — at the WAF, CDN, or origin. Report that as
a Critical finding and point the user at their edge configuration, not their
robots.txt.
