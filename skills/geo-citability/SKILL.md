...
```bash
# Score every passage on a page (0-100 per block, page average, grade mix)
seogeo citability https://example.com/guide --json

# Score a local HTML file instead of fetching
seogeo citability --file page.html --json

# Just the heading-delimited blocks, before scoring
seogeo blocks https://example.com/guide --min-words 20 --json
```

`citability` returns `average_citability_score`, `grade_distribution`,
`optimal_length_passages` (blocks in the 134-167 word band answer engines
lift whole), plus `top_5_citable` and `bottom_5_citable`. Rewrite the bottom
five first — they are the blocks dragging the page average down.

Each block carries a `breakdown`: answer-block quality (max 30),
self-containment (25), structural readability (20), statistical density (15),
uniqueness signals (10). Quote the failing component when recommending a
rewrite; "score 41" is not actionable, "self-containment 7/25 because every
sentence opens with a pronoun" is.
