...
`seogeo parse` classifies each image's lazy-loading mechanism via the
...
seogeo iptc audit ./images/ --json

# Audit a single image
seogeo iptc audit ./hero.webp --json

# Inject the AI label into an image
seogeo iptc inject ./ai-hero.webp \
...
seogeo images-audit --file page.html --json

# Label AI-generated imagery with IPTC DigitalSourceType
seogeo iptc audit ./images/ --json
seogeo iptc inject ./hero.webp --source-type trainedAlgorithmicMedia \
  --creator "Example Corp" --json
```

`images-audit` scores out of 100 and reports `missing_alt`,
`missing_dimensions`, `lazy_loaded`, `modern_format_images`, and
`lazy_methods` (which distinguishes native `loading="lazy"` from the
Perfmatters, EWWW, and generic JS lazy-loaders that WordPress stacks
install — a page reported as "not lazy-loaded" by a naive checker is
usually heavily lazy-loaded by one of these).

`first_image_lazy` deserves its own attention: the first image is usually
the LCP element, and lazy-loading it delays discovery and costs LCP
directly. That is a fix worth making before any format conversion.

`iptc` writes an XMP sidecar next to the image. To embed the tag into the
image container itself, merge the sidecar with exiftool — the command
