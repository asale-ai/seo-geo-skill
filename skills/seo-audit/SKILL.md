...
1. **Render homepage**: use `seogeo render <url> --mode auto --json` to capture raw HTML, rendered HTML, extracted text, SPA status, and accessibility data when needed
...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   ...
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (spawn when Google API credentials detected via `seogeo google-auth --check`)
   - `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (spawn when Moz or Bing API credentials detected via `seogeo backlinks-auth --check`, or always include Common Crawl domain-level metrics)
   ...
   - `seo-drift` -- Drift analysis: compare against stored baseline (spawn when drift baseline exists for the URL via `seogeo drift history <url>`)
...
- `{domain}-audit/screenshots/`: Desktop + mobile captures (if headless Chrome available)
- **PDF Report** (recommended): Generate a professional A4 PDF using `seogeo google-report --type full --data {domain}-audit/audit-data.json --domain <domain> --output-dir {domain}-audit/`. This produces a white-cover enterprise report with TOC, executive summary, charts (Lighthouse gauges, query bars, index donut), metric cards, threshold tables, prioritized recommendations with effort estimates, and implementation roadmap. Always offer PDF generation after completing an audit.

## Structured Audit Data Envelope

Write `{domain}-audit/audit-data.json` with this shape so `seogeo google-report --type full --data {domain}-audit/audit-data.json --domain <domain> --output-dir {domain}-audit/` can generate a report even when Google API data is unavailable:
...
The audit orchestrates the individual commands. Discovery first, then the
per-area checks in parallel:
...
# 2. Per-area checks (delegate to subagents)
seogeo parse --url https://example.com --json
seogeo schema-validate https://example.com --json
seogeo content-quality --json < page.html
seogeo images-audit https://example.com --json
seogeo preload-check https://example.com --json
seogeo citability https://example.com --json
seogeo agent-ux https://example.com --json

# 3. Deliverable
seogeo google-report --type full --data audit-data.json \
  --domain example.com --format pdf --output-dir ./example-audit/
```

Every command takes `--json`. Collect the JSON, not the human-readable
output — the summaries are lossy by design.
