...
```
1. seogeo render <url> --mode auto → raw/rendered HTML
2. seogeo parse --url <url>   → SEO elements
...
```bash
seogeo dataforseo-costs check merchant_google_products_search
```
...
After each call:
```bash
seogeo dataforseo-costs log merchant_google_products_search <cost>
```
...
seogeo dataforseo-merchant search "<keyword>" --marketplace google

# Seller analysis: merchant ratings and dominance
seogeo dataforseo-merchant sellers "<keyword>"

# Normalize results for analysis
seogeo dataforseo-normalize results.json --module merchant
```
...
```bash
seogeo dataforseo-costs check merchant_amazon_products_search
```
...
seogeo dataforseo-merchant search "<keyword>" --marketplace amazon

# Cross-marketplace comparison
seogeo dataforseo-merchant compare "<keyword>"
```
...
seogeo ucp-check https://store.example.com --json

# With endpoint reachability probes (HEAD each declared capability)
seogeo ucp-check https://store.example.com --probe-endpoints --json
```
...
```bash
# Merchant-listing readiness of the Product schema
seogeo schema-ecommerce https://example.com/product/widget --json

# Agentic-commerce readiness
seogeo ucp-check https://store.example.com --probe-endpoints --json

# Marketplace pricing and competition
seogeo dataforseo-merchant search "espresso machine" --marketplace google --json
seogeo dataforseo-merchant compare "espresso machine" --json
...
`schema-ecommerce` checks the stricter merchant-listing requirements, not
just Product rich results: a price, an ISO 4217 currency, a valid
`ItemAvailability` value, and a product identifier (`gtin*`, `mpn`, or
`sku`). `merchant_listing_eligible` is true only when there are zero
errors.

`ucp-check` reports a missing `/.well-known/ucp` profile as an
*opportunity*, not a failure. Adoption is early; the point of checking is
to surface a forward-looking signal, not to penalise a store for not
having one yet.

The DataForSEO commands bill per request. Run `dataforseo-costs check`
first and tell the user the estimate before spending their budget.
